...
(function (global) {
  ...
  function get_items(config, global_options) {
    ...
    if (typeof config === "string") {
      ...
      for (let idx in endpoints) config.wk_items.options[endpoints[idx]] = true;
    }
    ...
    for (let cfg_name in config) {
      ...
      remaining++;
      spec
        .fetcher(cfg, global_options)
        .then(function (data) {
          let filter_promise;
          if (typeof spec === "object") filter_promise = apply_filters(data, cfg, spec);
          else filter_promise = Promise.resolve(data);
  ...
  function get_wk_items(config, options) {
    ...
    for (let idx in available_endpoints) {
      ...
      if (ep_name === "subjects" || cfg_options[ep_name] === true)
        ep_promises.push(wkof.Apiv2.get_endpoint(ep_name, options).then(process_data.bind(null, ep_name)));
    }
    return Promise.all(ep_promises).then(function (all_data) {
      return all_data[0];
    });
  ...
  function apply_filters(items, config, spec) {
    ...
    for (let filter_name in config.filters) {
      let filter_cfg = config.filters[filter_name];
      if (typeof filter_cfg !== "object" || filter_cfg.value === undefined) filter_cfg = { value: filter_cfg };
    ...
    if (is_wk_items && options.include_hidden !== true) {
      filters.push({
        name: "remove_deleted",
        func: function (filter_value, item) {
          return item.data.hidden_at === null;
        },
        filter_value: true,
        invert: false,
      });
  ...
  function get_index(items, index_name) {
    ...
    if (typeof index_func !== "function")
      throw new Error('wkof.ItemData.index_by() - Invalid index function "' + index_name + '"');
  ...
  wkof.ItemData.registry.sources["wk_items"] = {
    ...
    options: {
      ...
      review_statistics: {
        ...
        default: false,
        hover_tip:
          'Include the "/review_statistics" endpoint:\n  * Per-item review count\n  *Correct/incorrect count\n  * Longest streak',
      },
      study_materials: {
        type: "checkbox",
    ...
    filters: {
      item_type: {
        ...
        filter_func: function (filter_value, item) {
          return filter_value[item.object] === true;
        },
      ...
      srs: {
        ...
        filter_func: function (filter_value, item) {
          return (
            filter_value[item.assignments && item.assignments.unlocked_at ? item.assignments.srs_stage : -1] === true
          );
        },
      ...
      have_burned: {
        ...
        filter_func: function (filter_value, item) {
          return (item.assignments !== undefined && item.assignments.burned_at !== null) === filter_value;
        },
        hover_tip:
          "Filter items by whether they have ever been burned.\n  * If checked, select burned items (including resurrected)\n  * If unchecked, select items that have never been burned",
      },
    },
  };
  ...
  function make_index_func(name, field, entry_type) {
    ...
    if (entry_type === "array") {
      ...
      if (entry_type === "single_or_array") {
        fn += "    if (!Array.isArray(index[value]))\n" + "        index[value] = [index[value]];\n";
      }
    }
    fn += "    index[value].push(item);\n" + "}\n" + "return index;";
  ...
  make_index_func(
    "srs_stage",
    "(item.assignments && item.assignments.unlocked_at ? item.assignments.srs_stage : -1)",
    "array",
  );
  make_index_func(
    "srs_stage_name",
    '(item.assignments && item.assignments.unlocked_at ? item.assignments.srs_stage_name : "Locked")',
    "array",
  );
  ...
  function levels_to_arr(filter_value) {
    ...
    for (crit_idx = 0; crit_idx < criteria.length; crit_idx++) {
      ...
      if (match !== null) {
        ...
        for (lvl = start; lvl <= stop; lvl++) levels[lvl] = value;
        continue;
      }
  ...
  function notify_ready() {
    ...
    setTimeout(function () {
      wkof.set_state("wkof.ItemData", "ready");
    }, 0);
  }
  wkof.include("Apiv2");

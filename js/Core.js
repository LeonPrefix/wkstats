...
(function (global) {
  ...
  function include(module_list) {
    if (wkof.get_state("wkof.wkof") !== "ready") {
      return wkof.ready("wkof").then(function () {
        return wkof.include(module_list);
      });
    }
  ...
  function load_file(url, use_cache) {
    ...
    function process_result(event) {
      ...
      if (use_cache) {
        file_cache_save(url, event.target.response).then(fetch_promise.resolve.bind(null, event.target.response));
      } else {
  ...
  function wait_state(state_var, value, callback, persistent) {
    ...
    if (persistent || value !== current_value)
      state_listeners[state_var].push({ callback: callback, persistent: persistent, value: value });

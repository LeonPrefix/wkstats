...
(function (global) {
  ...
  var show_popup = true;
  var popup_delay_started = false,
    popup_delay_expired = false,
    popup_timer;
  var externals_requested = false,
    externals_loaded = false;
  var progress_bars = {};
  ...
  function update_dialog() {
    ...
    for (name in progress_bars) {
      ...
      if (bar.length === 0) {
        bar = $(
          '<div class="wkof_progbar_wrap" name="' +
            name +
            '"><label>' +
            progress_bar.label +
            '</label><div class="wkof_progbar"></div></div>',
        );
  ...
  function set_ready_state() {
    ...
    setTimeout(function () {
      wkof.set_state("wkof.Progress", "ready");
    }, 0);
  }
  set_ready_state();
})(window);

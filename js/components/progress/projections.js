(function () {
      ...
      wkof.Apiv2.get_endpoint("level_progressions").then((levels) => {
        wkof.Apiv2.get_endpoint("spaced_repetition_systems").then((systems) => {
          wkof.ItemData.get_items("subjects, assignments").then((items) => {
            P.api(wkof.user, levels, systems, items);
          });
        });
      });
    });
  ...
  // COPIED CODE FROM PROJECTIONS ADDIN (https://community.wanikani.com/t/userscript-wkstats-projections-page/55260)
  ...
  const P = {
    ...
    project: function project() {
      ...
      const levels = P.progressions
        .slice()
        .concat(Array.from({ length: P.maxLevel - current.level + 2 }, (_, i) => ({ level: current.level + 1 + i })));
      const medianSpeed = P.median(
        P.progressions
          .slice(0, -1)
          .map(P.levelDuration)
          .sort((a, b) => a - b),
      ...
      for (const i of levels) {
        ...
        const l = i.level,
          ...
          _real = P.getLater(
            new Date(real || unlocked).add(l === P.maxLevel + 2 ? time[l - 1] : medianSpeed),
            _fastest,
          ),
          _given = P.getLater(
            new Date(given || unlocked).add(
              l === P.maxLevel + 2 ? time[l - 1] : P.getHyp(time[l - 1], l === current.level + 1),
            ),
            _fastest,
          );
        ...
        if (i.unlocked_at) {
          ...
          p[l] = {
            fastest: P.getFools((fastest = _fastest), fools).format(),
            real: (real = _real).format(),
            given: (given = _given).format(),
          };
          info = `<td> ${p[l].real} </td><td> ${p[l].fastest} </td><td> ${p[l].given} </td>`;
        } else {
          p[l] = { fastest: P.getFools(_fastest, fools).format(), real: _real.format(), given: _given.format() };
          info = `<td> ${p[l].real} </td><td> ${p[l].fastest} </td><td> ${p[l].given} </td>`;
        }

        if (!expanded) {
          output += `<tr ${i === current ? 'class="current_level"' : ""}><td> ${
            l === P.maxLevel + 2 ? "全火" : String("0" + l).slice(-2)
          } </td> ${info} </tr>`;
        } else if (expanded === i) {
      ...
      const time = function (item, burn) {
        if (!P.get(item.assignments, burn ? "burned_at" : "passed_at")) {
          let interval = P.get(item.assignments, "available_at")
            ? Math.max(0, new Date(item.assignments.available_at).subtractDate(P.now))
            : 0;
  ...
  // COPIED CODE FROM PROJECTIONS ADDIN
})();

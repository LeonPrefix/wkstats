(function () {
  document.title = "wkstats: JLPT Kanji";

  if (!wkof.user) return nav.login();

  wkof.load_script(wkof.support_files["calc_stats.js"], false);
  wkof.load_script(wkof.support_files["jlpt_kanji.js"], false);

  let settings_name = "items_jlpt";
  load_settings();
  wkof.ready("settings." + settings_name).then(init_query_ui);
  wkof.ready("wkdata.items,jlpt.kanji,settings." + settings_name).then(populate_items);
  ...
  let group_infos = {
    jlpt_level: {
      ...
      get_key: (item) => Number(item.level.slice(1)),
      get_order: (key) => -key,
      get_label: (key) => "N" + key,
    },
    wk_level: {
  ...
  let items = [],
    items_by_id = {};
  function get_items() {
    // Assemble a list of JLPT kanji.
    jlpt.kanji = [];
    jlpt.kanji_by_id = {};
    jlpt.kanji_by_level.forEach((level) => {
      let level_name = level.name;
      let slugs = level.kanji.split("");
      slugs.forEach((slug) => {
        let item = (jlpt.kanji_by_id[slug] = {});
        jlpt.kanji.push(item);
        item.id = slug;
        item.object = "kanji";
        ...
        } else if (jlpt.kanji_details[slug]) {
          item.is_on_wk = false;
          item.data = jlpt.kanji_details[slug];
          item.data.slug = slug;
          item.data.level = "---";
    ...
    items = jlpt.kanji;
    items_by_id = jlpt.kanji_by_id;
  }

  //================================================
  function populate_items() {
    let content = $('section[name="items-jlpt"] .items');
    content.html("");
  ...
  function item_info_show(e) {
    ...
    html += '<hr>JLPT Level: <span class="data">' + item.level + "</span><br>";

(function () {
  document.title = "wkstats: Frequency Coverage";
  ...
  wkof.ready(items_src + ",freq.kanji,settings." + settings_name).then(draw_chart);
  ...
  //================================================
  let data;
  function draw_chart(force_recalc) {
    let content = $('section[name="charts-freq"] .chart');
    content.html("");
  ...
  function process_items_snapshot() {
    ...
    let groups = [];
    let index = 0;
    let freq_group_size = settings.freq_size;
    while (true) {
      let kanji = freq.kanji_by_freq.substr(index * freq_group_size, freq_group_size);
      if (kanji.length === 0) break;
      groups.push({ name: "F" + (index + 1), kanji: kanji });
      index++;
    }
  ...
  function freq_size_changed() {
    ...
    if (value < 100) {
      value = 100;
      $('.main-content .query input[name="freq_size"]').val(value);
    }
    if (value === settings.freq_size) return;
    settings.freq_size = value;
    draw_chart(true /* force_recalc */);
    wkof.Settings.save(settings_name);
  }
})();

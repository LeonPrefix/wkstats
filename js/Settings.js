...
(function (global) {
  ...
  function config_to_html(context) {
    ...
    if (child_passback.tabs) html = assemble_pages(id, child_passback.tabs, child_passback.pages) + html;
    ...
    function parse_item(name, item, passback) {
      ...
      switch (item.type) {
        case "tabset":
          child_passback = {};
          for (cname in item.content) non_page += parse_item(cname, item.content[cname], child_passback);
          if (child_passback.tabs) html = assemble_pages(id, child_passback.tabs, child_passback.pages);
          break;
        ...
        case "page":
          ...
          passback.tabs.push(
            '<li id="' + id + '_tab"' + to_title(item.hover_tip) + '><a href="#' + id + '">' + item.label + "</a></li>",
          );
          child_passback = {};
          for (cname in item.content) non_page += parse_item(cname, item.content[cname], child_passback);
          if (child_passback.tabs) html = assemble_pages(id, child_passback.tabs, child_passback.pages);
        ...
        case "text":
          ...
          html += wrap_right(
            '<input id="' +
              ...
              name +
              '"' +
              (item.placeholder ? ' placeholder="' + escape_attr(item.placeholder) + '"' : "") +
              ">",
          );
          html = wrap_row(html, item.full_width, item.hover_tip);
          break;
    ...
    //============
    function assemble_pages(id, tabs, pages) {
      return '<div id="' + id + '" class="wkof_stabs"><ul>' + tabs.join("") + "</ul>" + pages.join("") + "</div>";
    }
    function wrap_row(html, full, hover_tip) {
      return '<div class="row' + (full ? " full" : "") + '"' + to_title(hover_tip) + ">" + html + "</div>";
    }
    function wrap_left(html) {
      return '<div class="left">' + html + "</div>";
    }
    function wrap_right(html) {
      return '<div class="right">' + html + "</div>";
    }
  ...
  function open(context) {
    ...
    dialog_elem.find("form").on("submit", function () {
      return false;
    });
    ...
    function resize(context, event, ui) {
      ...
      if (is_narrow && ui.size.width >= 510) dialog.removeClass("narrow");
      else if (!is_narrow && ui.size.width < 490) dialog.addClass("narrow");
    }
  ...
  function load_settings(context, defaults) {
    var script_id = typeof context === "string" ? context : context.cfg.script_id;
    return wkof.file_cache.load("wkof.settings." + script_id).then(finish, finish.bind(null, {}));

    function finish(settings) {
      if (defaults) wkof.settings[script_id] = deep_merge(defaults, settings);
      else wkof.settings[script_id] = settings;
  ...
  function save_btn(context, e) {
    ...
    var settings = wkof.settings[script_id];
    if (settings) {
      var active_tabs = dialog
        .find(".ui-tabs-active")
        .toArray()
        .map(function (tab) {
          return "#" + tab.attributes.id.value;
        });
  ...
  function setting_changed(context, event) {
    ...
    if (typeof valid === "boolean") valid = { valid: valid, msg: "" };
    else if (typeof valid === "string") valid = { valid: false, msg: valid };
    else if (valid === undefined) valid = { valid: true, msg: "" };
    switch (itype) {
      case "number":
        if (typeof item.min === "number" && Number(value) < item.min) {
          valid.valid = false;
          if (valid.msg.length === 0) {
            if (typeof item.max === "number") valid.msg = "Must be between " + item.min + " and " + item.max;
            else valid.msg = "Must be " + item.min + " or higher";
          }
        } else if (typeof item.max === "number" && Number(value) > item.max) {
          valid.valid = false;
          if (valid.msg.length === 0) {
            if (typeof item.min === "number") valid.msg = "Must be between " + item.min + " and " + item.max;
            else valid.msg = "Must be " + item.max + " or lower";
          }
        }
        if (!valid) break;
      ...
      case "text":
        if (item.match !== undefined && value.match(item.match) === null) {
          valid.valid = false;
          if (valid.msg.length === 0) valid.msg = item.error_msg || "Invalid value";
        }
        break;
    }
  ...
  //------------------------------
  var css_url = wkof.support_files["jqui_wkstats.css"];
  ...
  wkof
    ...
    .then(function (data) {
      ...
      setTimeout(function () {
        wkof.set_state("wkof.Settings", "ready");
      }, 0);
    });
})(this);
